(function () {
  "use strict";
  const SPRITE_URL = "./custom_module_monster_sprite_transparent.png";
  ...
  function init() {
    ...
    function maxX() { return Math.max(SIDE_MARGIN, window.innerWidth - FRAME_SIZE - SIDE_MARGIN); }
    function position() {
    ...
    function animate(timestamp) {
      ...
      if (row !== ROW_IDLE) {
        ...
        if (x <= SIDE_MARGIN) { x = SIDE_MARGIN; direction = 1; row = ROW_WALK_RIGHT; }
        if (x >= maxX()) { x = maxX(); direction = -1; row = ROW_WALK_LEFT; }
      }
    ...
    sprite.onload = function () { position(); draw(); requestAnimationFrame(animate); };
    sprite.onerror = function () { console.error("画像を読み込めませんでした:", SPRITE_URL); };
    window.addEventListener("resize", function () { x = Math.min(Math.max(x, SIDE_MARGIN), maxX()); position(); });
