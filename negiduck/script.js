(function () {
  ...
  function initNegiduck() {
    const canvas = document.getElementById("negiduckCanvas");
    const wrapper = document.querySelector(".negiduck-module");

    if (!canvas || !wrapper) {
      retryCount += 1;
      if (retryCount <= MAX_RETRY) {
        window.setTimeout(initNegiduck, 100);
      }
      return;
    }
    ...
    let lastFrameTime = 0;
    let lastTimestamp = 0;
    let behaviorUntil = performance.now() + 1800;
    ...
    sprite.onerror = function () {
      console.error("ネギダックのスプライト画像を読み込めませんでした:", SPRITE_URL);
    };
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initNegiduck);
  } else {
    initNegiduck();
  }
})();
