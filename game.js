...
function openRoster(fid=null){
 const target=fid===null?null:S.fleets.find(f=>f.id===fid),rows=S.people.filter(p=>p.alive!==false).map(p=>{const r=rankByXp(p.xp),assignedFleet=S.fleets.find(f=>f.id===p.assignment),isCurrent=target&&target.commander===p.id,available=!p.assignment||isCurrent;return `<tr><td><span class="rankBadge">${r[1]}</span></td><td><b>${r[0]} ${p.name}</b><span class="assignmentState ${available?'free':'busy'}">${isCurrent?'現在の司令官':assignedFleet?`${assignedFleet.name}に任命中`:'無任所'}</span></td><td>操艦 ${p.pilot}</td><td>武力 ${p.force}</td><td>外交 ${p.diplomacy}</td><td>${target?`<button data-assignperson="${p.id}" ${available&&!isCurrent?'':'disabled'}>${isCurrent?'任命中':'任命'}</button>`:''}</td></tr>`}).join('');
...
function assignCommander(fid,pid){
 const f=S.fleets.find(x=>x.id===fid),person=S.people.find(x=>x.id===pid&&x.alive!==false);if(!f||!person)return result('任命できません','艦隊または人物が見つかりません。',()=>render());
 ...
 result('司令官任命',`${rankByXp(person.xp)[0]} ${person.name}を${f.name}の司令官に任命しました。`,()=>{log(`${person.name}を${f.name}へ任命しました。`,'good');sel={kind:'fleet',id:f.id};route={fid:f.id,pid:f.at};render()});
}
...
function migrateState(){
 ...
 S.people.forEach(p=>{p.xp=Number(p.xp||0);p.alive=p.alive!==false;p.assignment=p.assignment??null});
}
...
function gainXp(f,amount,reason){const p=commander(f);if(!p)return;const before=rankByXp(p.xp)[0];p.xp+=amount;const after=rankByXp(p.xp)[0];log(`${p.name}が${reason}で経験値${amount}を獲得しました。`,'info');if(before!==after)result('昇進',`${p.name}は${after}へ昇進しました。`,()=>render())}
function commanderCasualty(f,reason){const p=commander(f);if(!p)return;p.alive=false;p.assignment=null;f.commander=null;log(`${p.name}は${reason}により死亡除隊となりました。`,'bad')}
